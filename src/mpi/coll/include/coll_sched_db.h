...
MPIC_INLINE void MPIC_add_sched(MPIC_sched_entry_t ** table, void *coll_args, int size, void *sched)
{
...
MPIC_INLINE void *MPIC_get_sched(MPIC_sched_entry_t * table, void *coll_args, int size)
{
    MPIC_sched_entry_t *s = NULL;
    MPL_HASH_FIND(handle, table, coll_args, size, s);
    if (s)
...
MPIC_INLINE void MPIC_delete_sched_table(MPIC_sched_entry_t * table, MPIC_sched_free_fn free_fn)
{
