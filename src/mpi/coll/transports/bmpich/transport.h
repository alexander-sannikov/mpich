...
static inline void MPIC_BMPICH_opinfo(MPIC_BMPICH_op_t op, int *is_commutative)
{
    MPIR_Op *op_ptr;
...
static inline void MPIC_BMPICH_addref_dt(MPIC_BMPICH_dt_t dt, int up)
{
...
static inline void MPIC_BMPICH_addref_op(MPIC_BMPICH_op_t op, int up)
{
    MPIR_Op *op_ptr;
...
static inline int MPIC_BMPICH_addref_dt_nb(MPIC_BMPICH_dt_t dt,
                                           int up,
                                           MPIC_BMPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
...
static inline int MPIC_BMPICH_queryfcn(void *data, MPI_Status * status)
{
    MPIC_BMPICH_recv_reduce_arg_t *rr = (MPIC_BMPICH_recv_reduce_arg_t *) data;
...
static inline int MPIC_BMPICH_dtcopy(void *tobuf,
                                     ...
                                     const void *frombuf, int fromcount, MPIC_BMPICH_dt_t fromtype)
{
...
static inline void MPIC_BMPICH_issue_request(int vtxid, MPIC_BMPICH_req_t * rp,
                                             MPIC_BMPICH_sched_t * sched)
{
    if (0)
    ...
    case MPIC_BMPICH_KIND_SEND:{
            ...
            MPIC_Send(rp->nbargs.sendrecv.buf,
                      ...
                      sched->tag, rp->nbargs.sendrecv.comm->mpid_comm, &errflag);
        }
        break;
    ...
    case MPIC_BMPICH_KIND_RECV:{
            ...
            MPIC_Recv(rp->nbargs.sendrecv.buf,
                      ...
                      rp->nbargs.sendrecv.dt,
                      rp->nbargs.sendrecv.dest,
                      sched->tag, rp->nbargs.sendrecv.comm->mpid_comm, &status, &errflag);
        }
        break;

    case MPIC_BMPICH_KIND_ADDREF_DT:
        MPIC_BMPICH_addref_dt(rp->nbargs.addref_dt.dt, rp->nbargs.addref_dt.up);
    ...
    case MPIC_BMPICH_KIND_ADDREF_OP:
        MPIC_BMPICH_addref_op(rp->nbargs.addref_op.op, rp->nbargs.addref_op.up);
    ...
    case MPIC_BMPICH_KIND_RECV_REDUCE:{
            ...
            MPIC_Recv(rp->nbargs.recv_reduce.inbuf,
                      ...
                      sched->tag, rp->nbargs.recv_reduce.comm->mpid_comm, &status, &errflag);
    ...
    case MPIC_BMPICH_KIND_REDUCE_LOCAL:
        if (rp->nbargs.reduce_local.flags == 0 ||
            rp->nbargs.reduce_local.flags & MPIC_FLAG_REDUCE_L) {
            MPIR_Reduce_local_impl(rp->nbargs.reduce_local.inbuf, rp->nbargs.reduce_local.inoutbuf,
                                   rp->nbargs.reduce_local.count, rp->nbargs.reduce_local.dt,
                                   rp->nbargs.reduce_local.op);
            if (1)
                fprintf(stderr, "  --> MPICH transport (reduce local_L) complete\n");
        }
        else {
            ...
            MPIR_Localcopy(rp->nbargs.reduce_local.inbuf,
                           ...
                           rp->nbargs.reduce_local.count, rp->nbargs.reduce_local.dt);
            if (1)
                fprintf(stderr, "  --> MPICH transport (reduce local_R) complete\n");
        }
