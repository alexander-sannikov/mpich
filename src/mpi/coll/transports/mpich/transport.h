...
static inline void MPIC_MPICH_record_vtx_completion(MPIC_MPICH_vtx_t * vtxp,
                                                    MPIC_MPICH_sched_t * sched)
{
...
static inline void MPIC_MPICH_allocate_vtx(MPIC_MPICH_vtx_t * vtx, int invtcs_array_size,
...
static inline void MPIC_MPICH_add_elems_int_array(MPIC_MPICH_int_array * in, int n_elems,
                                                  int *elems)
{
    if (0)
        fprintf(stderr, "here in add_elems_int_array, in->used: %d, in->size: %d\n", in->used,
                in->size);
    if (in->used + n_elems > in->size) {
        if (0)
            fprintf(stderr, "increasing int_array size\n");
    ...
    if (0)
        fprintf(stderr, "adding %d elems to int_array\n", n_elems);
    /*add new elements */
    memcpy(in->array + in->used, elems, sizeof(int) * n_elems);
    in->used += n_elems;
    if (0)
        fprintf(stderr, "done adding %d elems to int_array\n", n_elems);
}

/*this vertex sets the incoming edges (inedges) to vtx
and also add vtx to the outgoing edge list of vertices in inedges*/
static inline void MPIC_MPICH_add_vtx_dependencies(MPIC_MPICH_sched_t * sched, int vtx_id,
...
{
    if (0)
        fprintf(stderr, "TSP(mpich) : sched [wait_for] total=%ld \n", sched->total);
    MPIC_MPICH_vtx_t *vtxp;
    int vtx_id = MPIC_MPICH_get_new_vtx(sched, &vtxp);
    vtxp->kind = MPIC_MPICH_KIND_NOOP;
    MPIC_MPICH_init_vtx(sched, vtxp, vtx_id);
...
static inline void MPIC_MPICH_issue_vtx(int vtxid, MPIC_MPICH_vtx_t * rp,
                                        MPIC_MPICH_sched_t * sched)
{
        ...
        case MPIC_MPICH_KIND_REDUCE_LOCAL:
            if (rp->nbargs.reduce_local.flags == -1 ||
                ...
                MPIR_Reduce_local_impl(rp->nbargs.reduce_local.inoutbuf,
                                       rp->nbargs.reduce_local.inbuf, rp->nbargs.reduce_local.count,
                                       rp->nbargs.reduce_local.dt, rp->nbargs.reduce_local.op);
...
static inline int MPIC_MPICH_test(MPIC_MPICH_sched_t * sched)
{
    ...
    if (sched->issued_head == NULL) {
        if (0)
            fprintf(stderr, "issued list is empty, issue ready vtcs\n");
