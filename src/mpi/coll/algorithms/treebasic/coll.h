                             ...
                             int tree_type)
{
    ...
    if (comm_ptr->node_comm != NULL) {  /*MPICH is shared memory aware */
        ...
        if (comm_ptr->node_roots_comm != NULL &&
            comm_ptr->local_size > comm_ptr->node_comm->local_size) {
            if (0)
                fprintf(stderr, "starting internode bcast\n");
            MPID_Bcast(buffer, count, datatype, 0, comm_ptr->node_roots_comm, errflag);
            if (0)
                fprintf(stderr, "finished internode bcast\n");
        }
        /*Intranode bcast - performed by everyone */
        if (0)
            fprintf(stderr, "starting intranode bcast\n");
        MPID_Bcast(buffer, count, datatype, 0, comm_ptr->node_comm, errflag);
        if (0)
            fprintf(stderr, "finished intranode bcast\n");
    ...
    if (tree_type == 0) {       /*Do kary bcast */
        ...
        if (0)
            fprintf(stderr, "value of k for kary tree bcast = %d\n", k);
    }
    else if (tree_type == 1) {  /*Do knomial bcast */
        k = MPIR_CVAR_BCAST_KNOMIAL_KVAL;
        tree = &comm->knomial_tree[k];
        if (0)
            fprintf(stderr, "value of k for knomial tree bcast = %d\n", k);
    }
    else {
