...
COLL_brucks_pup(bool pack, void *rbuf, void *pupbuf, COLL_dt_t rtype, int count, \
    ...
    int *dtcopy_id = TSP_allocate_mem(sizeof(int)*comm_size); /**NOTE**: We do not need this large array - make it more accurate*/
    int counter=0;
    *pupsize = 0;/*points to the first empty location in pupbuf*/
    while(offset < comm_size){
        if(pack){
            dtcopy_id[counter++] = TSP_dtcopy_nb(pupbuf+*pupsize, count, rtype, rbuf+offset*count*extent, count, rtype, &s->tsp_sched, ninvtcs, invtcs);
            if(0) fprintf(stderr,"packing rbuf+%d to pupbuf+%d\n",offset*count*extent,*pupsize);
        }
        else{
            dtcopy_id[counter++] = TSP_dtcopy_nb(rbuf+offset*count*extent, count, rtype, pupbuf+*pupsize, count, rtype, &s->tsp_sched, ninvtcs, invtcs);
                    ...
                    COLL_comm_t *comm, int tag, COLL_sched_t *s, int k)
{
    ...
    int *pack_invtcs = (int*)TSP_allocate_mem(sizeof(int)*k);      int pack_ninvtcs;
    int *send_invtcs = (int*)TSP_allocate_mem(sizeof(int)*k);      int send_ninvtcs;
    int *recv_invtcs = (int*)TSP_allocate_mem(sizeof(int)*k);      int recv_ninvtcs;
    int *unpack_invtcs = (int*)TSP_allocate_mem(sizeof(int)*k);    int unpack_ninvtcs;


    if(0) fprintf(stderr,"here \n");
    ...
    if(0) fprintf(stderr,"alltoall_brucks: num_ranks: %d, k: %d \n", size, k);
    ...
    for(i=0; i<k-1; i++) {
        tmp_sbuf[i] = TSP_allocate_buffer((int)r_extent*recvcount*ceil((float)size/k), &s->tsp_sched);
        tmp_rbuf[i] = TSP_allocate_buffer((int)r_extent*recvcount*ceil((float)size/k), &s->tsp_sched);
    }
    ...
    */
    ...
    for(i=0; i<nphases; i++) {
        for(j=1; j<k; j++) { /*for every non-zero value of digitval*/
            ...
            if(i!=0){pack_invtcs[k-1]=sendids[j-1]; pack_ninvtcs=k;}
            packids[j-1] = COLL_brucks_pup(1,recvbuf,tmp_sbuf[j-1],recvtype,recvcount,i,k,j,size,&packsize,s,pack_ninvtcs, pack_invtcs);
            *unpack_invtcs = packids[j-1]; unpack_ninvtcs=1;
            ...
            if(i!=0){*recv_invtcs = unpackids[j-1]; recv_ninvtcs=1;}
            recvids[j-1] = TSP_recv(tmp_rbuf[j-1],packsize,*pack_dt,src,tag,&comm->tsp_comm,&s->tsp_sched,recv_ninvtcs, recv_invtcs);
            if(0) fprintf(stderr,"phase %d, digit %d recv scheduled\n", i,j);
            
            *(unpack_invtcs+1) = recvids[j-1]; unpack_ninvtcs=2;
            unpackids[j-1] = COLL_brucks_pup(0,recvbuf,tmp_rbuf[j-1],recvtype,recvcount,i,k,j,size,&packsize,s,unpack_ninvtcs,unpack_invtcs);
            if(0) fprintf(stderr,"phase %d, digit %d unpacking scheduled\n", i,j);
        }
        TSP_dtcopy(pack_invtcs,sizeof(int)*(k-1),*pack_dt,unpackids,sizeof(int)*(k-1),*pack_dt);
        pack_ninvtcs=k-1;
