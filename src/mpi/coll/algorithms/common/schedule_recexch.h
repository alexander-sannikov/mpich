                     ...
                     int finalize)
{
    ...
    int **step2_nbrs;
    int p_of_k, T;
    if(0) fprintf(stderr,"Beforeinitial dt copy\n");
    /*get the neighbors*/
        ...
        for(i=0; i<k-1; i++) {
           if(count==0)
            ...
            send_id[i] = TSP_send(tmp_buf,count,datatype,
                                     nbr,tag,tsp_comm,sched,nvtcs,vtcs);
            if(rank > nbr){
                 myidx = i+1;
            }
        }
...
COLL_sched_allgather_recexch_data_exchange(int rank,
        ...
        if(0) printf("data exchange with %d send_offset %d count %d \n", partner, send_offset, count);
...
COLL_sched_allgather_recexch_step1(int step1_sendto,
    ...
    if(step1_sendto !=-1){/*non-participating rank sends the data to a partcipating rank*/
        ...
        void *buf_to_send;
        if(is_inplace) buf_to_send = ((char*)recvbuf+send_offset);
        else buf_to_send = sendbuf;
        TSP_send(buf_to_send,recvcount,recvtype,step1_sendto,
                           tag,&comm->tsp_comm,&s->tsp_sched,0,NULL);
        ...
        for(i=0; i<step1_nrecvs; i++){/*participating rank gets the data from non-participating rank */
            ...
            TSP_recv(((char*)recvbuf+recv_offset),recvcount,recvtype,
                            step1_recvfrom[i],tag,&comm->tsp_comm,&s->tsp_sched,n_invtcs,invtx);
        }
    }
}
...
COLL_sched_allgather_recexch_step2(int step1_sendto,
    ...
    for(j=0; j<step2_nphases && step1_sendto==-1; j++){
        ...
        for(i=0; i<k-1; i++) {
            nbr = step2_nbrs[phase][i];
            if (dist_halving==1) rank_for_offset = reverse_digits(rank,nranks,k,p_of_k,log_pofk,T);
            else rank_for_offset = rank;
            ...
            TSP_send(((char*)recvbuf+send_offset),count*recvcount,recvtype,
                                    nbr,tag,&comm->tsp_comm,&s->tsp_sched,nrecvs,recv_id);
            if(0)
                printf("phase %d nbr is %d send offset %d count %d depend on %d \n",phase,nbr,send_offset,count,((k-1)*j));
         }
        /*receive from the neighbors */
        for(i=0; i<k-1; i++){
            nbr = step2_nbrs[phase][i];
            if (dist_halving==1) rank_for_offset = reverse_digits(nbr,nranks,k,p_of_k,log_pofk,T);
            else rank_for_offset = nbr;
            allgather_get_offset_and_count(rank_for_offset,j,k,nranks,&count, &offset);
            recv_offset = offset*recv_extent*recvcount;
            recv_id[j*(k-1)+i]= TSP_recv(((char*)recvbuf+recv_offset),count*recvcount,recvtype,nbr,
                                          tag,&comm->tsp_comm,&s->tsp_sched,0,NULL);
            nrecvs++;
            if(0)
                printf("phase %d recv from %d recv offset %d cur_count %d recv returns id[%d] %d\n",phase,nbr,recv_offset,count, j*(k-1)+i, recv_id[j*(k-1)+i]);
        }
        if(dist_halving == 1)
            phase--;
        else
...
COLL_sched_allgather_recexch_step3(int step1_sendto,
    ...
    int i;
    if(step1_sendto!= -1){
        TSP_recv(recvbuf,recvcount*nranks,recvtype,step1_sendto,tag,&comm->tsp_comm,&s->tsp_sched,0,NULL);
    }

    for(i=0;i<step1_nrecvs;i++){
       TSP_send(recvbuf,recvcount*nranks,recvtype,step1_recvfrom[i],
                                tag,&comm->tsp_comm,&s->tsp_sched,nrecvs,recv_id);
    }
}
                           ...
                           int dist_halving)
{
    ...
    int dtcopy_id;
    int n_invtcs=0; int invtx;
    if(!is_inplace && is_instep2){
         dtcopy_id = TSP_dtcopy_nb((char*)recvbuf+rank*recv_extent*recvcount,recvcount,recvtype,
                           sendbuf,recvcount,recvtype,&s->tsp_sched,0,NULL); /*copy the data to recvbuf but only if you are a rank participating in Step 2*/
    }
    /*step 1*/
    n_invtcs=(is_inplace)?0:1;
    invtx=dtcopy_id;
    COLL_sched_allgather_recexch_step1(step1_sendto, step1_recvfrom, step1_nrecvs, is_inplace, rank, tag,  sendbuf, recvbuf, recv_extent, recvcount, recvtype, n_invtcs,&invtx, comm, s);
    ...
    if(dist_halving == 1){
        if(step1_sendto==-1) {
            COLL_sched_allgather_recexch_data_exchange(rank,nranks,k,p_of_k,log_pofk, T, recvbuf,recvtype,recv_extent,recvcount,tag,comm,s);
            TSP_wait(&s->tsp_sched);
        }
    }
    int nrecvs;
    ...
    /* step2 */
    COLL_sched_allgather_recexch_step2(step1_sendto,step2_nphases,step2_nbrs,rank,nranks,k,p_of_k, log_pofk, T, &nrecvs, &recv_id,tag,recvbuf, recv_extent,recvcount,recvtype,comm,s,dist_halving);
    /*Step 3: This is reverse of Step 1. Ranks that participated in Step 2
    send the data to non-partcipating ranks*/
    COLL_sched_allgather_recexch_step3(step1_sendto, step1_recvfrom, step1_nrecvs, step2_nphases, recvbuf, recvcount, nranks, k, nrecvs, recv_id, tag, recvtype, comm, s);
    /* free the memory */
    for(i=0;i<step2_nphases;i++)
        TSP_free_mem(step2_nbrs[i]);
    TSP_free_mem(step2_nbrs);
    ...
    if(0) printf(stderr,"freed step1_recvfrom\n");
