...
static inline
int orgrank_to_step2rank(int rank,int rem,int T,int k){
     int step2rank;
...
static inline
int step2rank_to_orgrank(int rank,int rem,int T,int k){
    int orig_rank;
...
static inline
int allgather_get_offset_and_count(int rank, int phase, int k, int nranks, int *count, int* offset){
     int step2rank, min, max, orig_max,orig_min;
     /* k_power_phase is k^p */
     int k_power_phase=1;

    /*p_of_k is the largest power of k that is less than nranks*/
    int p_of_k = 1,log_p_of_k=0,rem,T;
    while(p_of_k <= nranks){
        p_of_k *= k;
    }
    p_of_k /= k;
...
static inline
int reverse_digits(int rank, int comm_size,int k, int pofk, int log_pofk, int T){
